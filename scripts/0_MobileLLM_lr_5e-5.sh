...
# Determine noise suffix
if [ "$NOISE_INJECTION" = "False" ] && [ "$PRE_QUANTIZATION_NOISE" = "False" ]; then
    NOISE_STR="wonoise"
else
    NOISE_STR="withnoise"
fi
...
echo "  - Share Embedding: $SHARE_EMBEDDING (MobileLLM only)"
echo "  - Layer Sharing: $LAYER_SHARING (MobileLLM only)"
echo ""
