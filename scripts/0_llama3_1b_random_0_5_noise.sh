...
# Format noise injection flag (True -> noise, False -> no_noise)
if [ "$NOISE_INJECTION" = "True" ]; then
    NOISE_STR="noise"
else
    NOISE_STR="no_noise"
fi
...
# Generate EXP_NAME
EXP_NAME="${MODEL_NAME}_${BIT_STR}bit_${STEPS_STR}_bs${TOTAL_BS}_${LR_STR}_rap${RAP_STR}_prob${PROB_STR}_${NOISE_STR}_${DATE_STR}"
...
--noise_injection $NOISE_INJECTION ; then
