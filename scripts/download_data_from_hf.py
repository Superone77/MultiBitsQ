...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='从HuggingFace下载数据集并保存为JSONL')
    ...
    parser.add_argument('--subset_name', type=str, default='sample-100BT',
                        help='子集名称（默认：sample-100BT，使用None表示不使用子集）')
    ...
    # 流式加载数据集（无需下载完整数据）
    print(f"1/2 开始流式加载数据集：{args.dataset_name}" + (f"（{args.subset_name}）" if args.subset_name and args.subset_name.lower() != "none" else ""))
    
    # HuggingFace datasets使用name参数指定子集，如果subset_name为None或空字符串则不使用
    load_kwargs = {
    ...
    # 只有当subset_name不为None且不为空时才添加name参数
    if args.subset_name and args.subset_name.lower() != "none":
        load_kwargs["name"] = args.subset_name
    
    fw_dataset = load_dataset(**load_kwargs)
